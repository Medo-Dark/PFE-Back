...
class BaseRouter(Generic[ModelType, ResponseType]):
    def __init__(self, responseType: Type[ResponseType], repository: CRUDRepository[ModelType]):
        ...
        @self.router.get("/all", response_model=list[responseType], name=f'All {responseType.__name__}s')
        ...
        @self.router.get("/{resource_id}", response_model=responseType,
        ...
        @self.router.get("/username/{name}", response_model=responseType,
